...
static void orion_sata_phy_disable(struct clk_hw *hw)
{
	...
	reg = readl(data->io + SATA_PHY_MODE_2);
	writel(reg & ~0xf, data->io + SATA_PHY_MODE_2);
	/* Disable PHY */
	reg = readl(data->io + SATA_IF_CTRL);
	writel( reg | 0x200, data->io + SATA_IF_CTRL);
}
...
static void orion_pcie_phy_disable(struct clk_hw *hw)
{
	...
	reg = readl(data->io + PCIE_LINK_CTRL);
	writel( reg | 0x10, data->io + PCIE_LINK_CTRL);
	while (1)
		    ...
		    struct clk *tclk)
{
	...
	cl = clkdev_alloc(tclk, NULL, __clk_get_name(tclk));
	if (cl)
		clkdev_add(cl);
	...
	for(n=0; n < data->num_gates; n++) {
		...
		if (phy_ops) {
			...
			phy = kzalloc(sizeof(struct orion_phy_gate),
				      GFP_KERNEL);
			if (phy == NULL) {
				printk("orion_clk: unable to allocate phy "
				       "gate %s\n", data->gates[n].name);
				return;
			}
