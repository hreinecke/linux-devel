				  ...
				  unsigned long *parent_rate)
{
	...
	rate = si5351_multisync_find_parameters(parent_rate, rate,
						&p1, &p2, &p3);
	...
	si5351_dbg("%s : prate = %lu => rate = %lu, p1 = %lu, "
		   "p2 = %lu, p3 = %lu\n", hw->clk->name,
		   parent_rate, rate, p1, p2, p3);
...
static int si5351_clkout_prepare(struct clk_hw *hw)
{
	...
	si5351_dbg("name = %s, ctrl = %02x, powerdown = %d, oectrl = %02x, "
		   "disabled = %d\n", hw->clk->name, hwdata->params.ctrl,
...
static int si5351_clkout_set_parent(struct clk_hw *hw, u8 index)
{
	...
	hwdata->sidata->pll[0].params.ctrl &= ~(1 << hwdata->num);
	hwdata->sidata->pll[1].params.ctrl &= ~(1 << hwdata->num);
				     ...
				     unsigned long *parent_rate)
{
	...
	si5351_dbg("%s : prate = %lu => rate = %lu, p1 = %lu, p2 = %lu, "
		   "p3 = %lu, divby4 = %d\n", hw->clk->name,
		   prate, rate, p1, p2, p3, divby4);
				  ...
				  unsigned long parent_rate)
{
	...
	if (hwdata->num >= 6) {
		...
		si5351_regs_write(hwdata->sidata, SI5351_CLK0_PARAMETERS +
				  hwdata->num * SI5351_PARAMETERS_LENGTH,
				  SI5351_PARAMETERS_LENGTH, &params);
...
static void si5351_init(struct si5351_driver_data *sidata)
{
	...
	for(n=0; n < 6; n++) {
		...
		si5351_dbg("clkout%d : ctrl = %02x, p1 = %lu, p2 = %lu, "
			   "p3 = %lu, divby4 = %d, rdiv = %d, drive = %d mA, "
			   "pdown = %d, disabled = %d\n",
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	hw = &sidata->pll[0].hw;
	hw->init = &init;
	clk = clk_register(&client->dev, hw);
	if (IS_ERR(clk)) {
	...
	for(i=0; i<num_clocks; i++) {
		...
		cl = clkdev_alloc(clk, si5351_clkout_names[i],
				  dev_name(&client->dev));
		if (cl)
			clkdev_add(cl);
	}
